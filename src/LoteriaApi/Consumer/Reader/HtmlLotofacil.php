...
class HtmlLotofacil extends AbstractReaderHtmlLoteria
{
    public function getData()
    {
        $this->numbersNode->setNumberConcurso(0)
            ...
            ->setTotalGanhadoresPrimeiroPremio(18)
            ->setTotalGanhadoresSegundoPremio(21)
            ->setTotalGanhadoresTerceiroPremio(22)
            ->setValorGanhadoresPrimeiroPremio(25)
            ->setValorGanhadoresSegundoPremio(26)
            ->setValorGanhadoresTerceiroPremio(27)
            ->setValorAcumuladoConcurso(30)
            ->setValorEstimadoProximoConcurso(31);
    ...
    public function getDataLive() {
        ...
        // Dezenas
        $nodes = $finder->query("//table[contains(@class, 'simple-table')][contains(@class, 'lotofacil')]/tbody/tr/td");
        $dezenas = parent::loadDezenasLive($nodes);

        // Arrecadacao
        $nodes = $finder->query("//div[contains(@class, 'related-box')][contains(@class, 'gray-text')]/p/strong");
        $arrecadacao = trim(preg_replace('/[^0-9,\.]/si', '', $nodes->item(6)->nodeValue));
